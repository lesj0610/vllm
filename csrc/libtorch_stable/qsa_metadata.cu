...
__global__ void qsa_metadata_kernel(
    ...
    int64_t* __restrict__ logical_positions,
    int32_t* __restrict__ visible_blocks, int64_t* __restrict__ slot_mapping,
  ...
  // How many compressed blocks this token is allowed to score: its own causal
  // prefix, capped by what the request has actually compressed. An unmapped
  // token has position -1 and gets zero.
  const int32_t position_blocks =
      POW2 ? (position + 1) >> ratio_shift : (position + 1) / compress_ratio;
  const int32_t sequence_blocks =
      POW2 ? seq_len >> ratio_shift : seq_len / compress_ratio;
  visible_blocks[token] = max(0, min(position_blocks, sequence_blocks));
...
void qsa_build_metadata(const Tensor& query_start_loc, const Tensor& seq_lens,
                        ...
                        Tensor& logical_positions, Tensor& visible_blocks,
                        Tensor& slot_mapping,
  ...
  STD_TORCH_CHECK(visible_blocks.size(0) == num_tokens,
                  "visible_blocks must cover every token");
  ...
  check_dtype(visible_blocks, ScalarType::Int, "visible_blocks");
  ...
  // The model runner calls this from build_for_cudagraph_capture() on a
  // dedicated capture stream, so the launch has to follow the caller's stream
  // rather than the legacy default one: anything else breaks capture and drops
  // ordering against the metadata uploads and the consumers.
  const torch::stable::accelerator::DeviceGuard device_guard(
      token_to_req.get_device_index());
  ...
  // A step with no tokens and no work items has nothing to launch, and a
  // zero-block grid is not a legal launch.
  if (token_blocks + work_blocks == 0) return;
  ...
  auto launch = [&](auto circular_tag, auto pow2_tag) {
    ...
    qsa_metadata_kernel<CIRCULAR, POW2><<<grid, kTokenBlock, 0, stream>>>(
        ...
        reinterpret_cast<int32_t*>(visible_blocks.data_ptr()),
