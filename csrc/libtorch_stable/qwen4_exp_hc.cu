...
__global__ void hc_silu_kernel(const T* __restrict__ x, T* __restrict__ y,
                               int64_t total, int64_t vecs_per_row,
                               int64_t stride_x, int64_t stride_y,
                               float inv_hc) {
  // The default model hands over a split view of a merged projection, whose
  // row stride is wider than the row. Walking rows and columns separately
  // costs one divide per element and keeps the vector loads aligned, which a
  // flat walk over a padded buffer would not.
  for (int64_t idx = blockIdx.x * blockDim.x + threadIdx.x; idx < total;
       idx += static_cast<int64_t>(gridDim.x) * blockDim.x) {
    const int64_t row = idx / vecs_per_row;
    const int64_t col = (idx - row * vecs_per_row) * kVec;
    const Vec<T> v = *reinterpret_cast<const Vec<T>*>(x + row * stride_x + col);
    Vec<T> out;
#pragma unroll
    for (int j = 0; j < kVec; ++j) {
    ...
    *reinterpret_cast<Vec<T>*>(y + row * stride_y + col) = out;
  }
}
...
void qwen4_exp_hc_silu(const Tensor& x, int64_t hc_count, Tensor& y) {
  ...
  // A split view of a merged projection has a row stride wider than the row,
  // so the strides are carried rather than assumed.
  const int64_t vecs_per_row = dim / kVec;
  const int64_t total = rows * vecs_per_row;
  const int grid = elementwise_grid(total, kFlatBlock);
  QWEN4EXP_HC_DISPATCH(x.scalar_type(), {
    hc_silu_kernel<scalar_t><<<grid, kFlatBlock, 0, stream>>>(
        reinterpret_cast<const scalar_t*>(x.data_ptr()),
        reinterpret_cast<scalar_t*>(y.data_ptr()), total, vecs_per_row,
        x.stride(0), y.stride(0), 1.f / static_cast<float>(hc_count));
  });
}
    ...
    auto go = [&](auto hc_tag) {
      auto launch = [&](auto vec_tag) {
        hc_combine_kernel<scalar_t, decltype(hc_tag)::value,
                          decltype(vec_tag)::value><<<grid, block, 0, stream>>>(
