...
class Worker(WorkerBase):
    ...
    def _validate_ple_offload_config(self) -> None:
        ...
        if not self.vllm_config.use_v2_model_runner:
            # Qwen4Exp's QSA state caches only have a model runner V2 path, so
            # the offload client is only wired into that runner.
    ...
    def load_model(self, *, load_dummy_weights: bool = False) -> None:
        ...
        # Model loading constructs the PLE placeholders whose CUDA buffers the
        # offload client registers. Only the V2 runner reaches here: model
        # runner V1 is rejected in _validate_ple_offload_config.
        if self._ple_offload_enabled:
            cast(Any, self.model_runner)._setup_ple_offload(
