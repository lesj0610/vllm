...
Two kernels:
1. _tq_fused_store_fp8: FP8 key scatter + value quantization.
2. _tq_fused_store_mse: Fused binary-search bucketize + MSE index
   packing for MSE keys. Current presets store values through the rotated
   MSE path; the legacy uniform helper remains available for explicit
   fallback paths.
...
@triton.jit
def _store_mse_value(
    ValueY_ptr,
    ValueNorms_ptr,
    ValueMidpoints_ptr,
    KV_cache_ptr,
    base,  # pid * D offset into ValueY_ptr
    slot_base,  # byte offset into KV_cache_ptr for this slot+head
    d_offs,  # tl.arange(0, BLOCK_D)
    d_mask,  # d_offs < D
    D: tl.constexpr,
    ...
    VAL_DATA_BYTES: tl.constexpr,
    BLOCK_D: tl.constexpr,
    N_VALUE_CENTROIDS: tl.constexpr,
    BLOCK_GRP: tl.constexpr,
):
    """MSE quantization of rotated normalized values plus fp16 norm."""
    val_cache_offset = KPS

    y_vec = tl.load(ValueY_ptr + base + d_offs, mask=d_mask, other=0.0)
    lo = tl.zeros([BLOCK_D], dtype=tl.int32)
    hi = tl.full([BLOCK_D], N_VALUE_CENTROIDS - 1, dtype=tl.int32)
    for _ in range(VQB):
        mid = (lo + hi) >> 1
        safe_mid = tl.minimum(mid, N_VALUE_CENTROIDS - 2)
        mid_val = tl.load(ValueMidpoints_ptr + safe_mid, mask=d_mask, other=0.0)
        lo = tl.where(y_vec >= mid_val, mid + 1, lo)
        hi = tl.where(y_vec >= mid_val, hi, mid)
    idx = tl.minimum(lo, N_VALUE_CENTROIDS - 1)
    ...
    norm_offset = val_cache_offset + VAL_DATA_BYTES
    vn_f16 = tl.load(ValueNorms_ptr + (base // D)).to(tl.float16)
...
# ═══════════════════════════════════════════════════════════════════════
# FP8 key store + value quantization
# ═══════════════════════════════════════════════════════════════════════
...
def _tq_fused_store_fp8(
    ...
    """FP8 key cast+scatter + legacy value uniform quantization."""
    pid = tl.program_id(0)
    token_idx = pid // H
    head_idx = pid % H
...
@triton.jit
def _tq_fused_store_fp8_value_mse(
    Key_ptr,  # [NH, D] float16/bfloat16 — raw keys
    ValueY_ptr,  # [NH, D] float32 — rotated normalized values
    ValueNorms_ptr,  # [NH] float32 — value vector norms (||v||)
    ...
    N_VALUE_CENTROIDS: tl.constexpr,
    BLOCK_GRP: tl.constexpr = 16,
    FP8_E4B15: tl.constexpr = 0,  # 1 = e4b15, 0 = e4nv
):
    """FP8 key cast+scatter + rotated-MSE value quantization."""
    pid = tl.program_id(0)
    token_idx = pid // H
    head_idx = pid % H
    ...
    _store_mse_value(
        ...
        VAL_DATA_BYTES=VAL_DATA_BYTES,
        BLOCK_D=BLOCK_D,
        N_VALUE_CENTROIDS=N_VALUE_CENTROIDS,
        BLOCK_GRP=BLOCK_GRP,
    )
...
def _tq_fused_store_mse(
    ...
    Value_ptr,  # [NH, D] float32 — raw values or rotated normalized values
    ValueNorms_ptr,  # [NH] float32 — value vector norms (only for VALUE_MSE)
    # Quantization tables
...
def triton_turboquant_store(
    ...
    key_fp8: bool = False,
    value_midpoints: torch.Tensor | None = None,
    ...
    if key_fp8:
        ...
        if value_mse:
            ...
            v_store = v_hat @ PiT
            _tq_fused_store_fp8_value_mse[grid](
                k_flat,
                v_store,
                v_norms.squeeze(1),
                value_midpoints,
                ...
                VAL_DATA_BYTES=val_data_bytes,
                N_VALUE_CENTROIDS=2**value_quant_bits,
                BLOCK_GRP=block_grp,
                FP8_E4B15=fp8_e4b15,
                num_warps=4,
                num_stages=1,
