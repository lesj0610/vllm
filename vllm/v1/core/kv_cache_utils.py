...
def get_max_concurrency_for_kv_cache_config(
    ...
    elif kv_cache_config.is_multi_pool:
        pool_required_blocks: dict[int, int] = defaultdict(int)
        for group_id, group in enumerate(kv_cache_config.kv_cache_groups):
            pool_id = kv_cache_config.group_to_pool_id[group_id]
            page_size = group.kv_cache_spec.page_size_bytes
            pool_required_blocks[pool_id] += cdiv(
                group.kv_cache_spec.max_memory_usage_bytes(vllm_config),
                page_size,
            )
        pool_concurrencies = []
        for pool in kv_cache_config.pool_configs:
            required = pool_required_blocks.get(pool.pool_id, 0)
            if required > 0:
...
def unify_kv_cache_spec_page_size(
    ...
    for layer_name, layer_spec in kv_cache_spec.items():
        if layer_spec.page_size_bytes == max_page_size:
            ...
            if isinstance(layer_spec, (TQFullAttentionSpec, TQSlidingWindowSpec)):
...
def get_kv_cache_config_from_groups(
    ...
    elif _is_multi_pool_kv_cache_groups(kv_cache_groups):
...
def _is_multi_pool_kv_cache_groups(
    ...
    return len({group.kv_cache_spec.page_size_bytes for group in kv_cache_groups}) > 1
...
def _collect_multi_pool_layout(
    ...
    pool_page_sizes = {
        pool_id: page_size for page_size, pool_id in page_size_to_pool_id.items()
    }
    ...
    group_to_pool_id: list[int] = []

    for group in kv_cache_groups:
        page_size = group.kv_cache_spec.page_size_bytes
        pool_id = page_size_to_pool_id[page_size]
        group_to_pool_id.append(pool_id)
        pool_blocks_per_request[pool_id] += cdiv(
            group.kv_cache_spec.max_memory_usage_bytes(vllm_config), page_size
        )
        pool_group_sizes[pool_id] = max(
            pool_group_sizes[pool_id], len(group.layer_names)
        )
        for position, layer_name in enumerate(group.layer_names):
            pool_layers_by_position[pool_id][position].append(layer_name)
...
def _get_kv_cache_config_multi_pool(
    ...
    pool_configs: list[KVCachePoolConfig] = []
    kv_cache_tensors: list[KVCacheTensor] = []
    group_ids_by_pool: dict[int, list[int]] = defaultdict(list)
    for group_id, pool_id in enumerate(layout.group_to_pool_id):
    ...
    for pool_id in sorted(layout.pool_page_sizes):
        ...
        pool_configs.append(
            KVCachePoolConfig(
                ...
                group_ids=tuple(group_ids_by_pool[pool_id]),
                num_blocks=num_blocks,
                accounting_page_size_bytes=page_size,
                physical_page_size_bytes=page_size,
            )
        )
    ...
    return KVCacheConfig(
        ...
        kv_cache_groups=kv_cache_groups,
        pool_configs=pool_config_tuple,
        group_to_pool_id=layout.group_to_pool_id,
    )
...
def _get_kv_cache_groups_multi_pool_candidate(
    ...
    multi_pool_bytes = _max_memory_usage_bytes_multi_pool(vllm_config, candidate_groups)
