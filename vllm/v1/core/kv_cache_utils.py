...
def _get_effective_available_memory_for_num_blocks_override(
    vllm_config: VllmConfig,
    kv_cache_groups: list[KVCacheGroupSpec],
    available_memory: int,
    num_blocks_override: int,
) -> tuple[int, int]:
    """Return the effective memory budget represented by a block override.

    `num_gpu_blocks_override` clamps only TOKEN_PROPORTIONAL pools. Mixed memory
    models still need their REQUEST_CONSTANT pools reserved separately, so the
    effective byte budget is reservation bytes plus the requested number of
    token-proportional blocks.
    """
    ...
    if _has_request_constant_groups(kv_cache_groups):
        ...
        bytes_per_block = _pool_bytes_per_block(token_groups) if token_groups else 0
    else:
        request_constant_bytes = 0
        bytes_per_block = _pool_bytes_per_block(kv_cache_groups)

    profiled_blocks = available_memory // bytes_per_block if bytes_per_block else 0
    effective_available_memory = (
        request_constant_bytes + num_blocks_override * bytes_per_block
    )
    return effective_available_memory, profiled_blocks
...
def unify_kv_cache_spec_page_size(
    ...
    size by increasing the block size of layers with smaller page size. If the
    page sizes are not divisible, pad the smaller page when the spec supports
    `page_size_padded`. Raise NotImplementedError if failed to unify the page
    size.
    ...
    for layer_name, layer_spec in kv_cache_spec.items():
        if layer_spec.page_size_bytes == max_page_size:
            ...
            if max_page_size % layer_page_size != 0:
                if isinstance(layer_spec, (AttentionSpec, MambaSpec)):
                    new_spec = replace(layer_spec, page_size_padded=max_page_size)
                else:
                    raise NotImplementedError(
                        "The page size of the layer is not divisible by the "
                        "maximum page size and the spec does not support page "
                        "padding."
                    )
            else:
...
def _get_kv_cache_config_mixed_memory_model(
    ...
    if reserved_bytes > available_memory or (
        token_group_ids and reserved_bytes >= available_memory
    ):
        raise ValueError(
...
def get_kv_cache_configs(
    ...
    if override is not None:
        ...
        for groups, avail_mem in zip(projected_groups_per_worker, available_memory):
            ...
            adjusted_memory.append(adjusted_mem)
