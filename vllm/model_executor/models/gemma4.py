...
def _infer_autoround_gptq_router_config(
    ...
    if input_size % qweight.shape[0] != 0:
        raise ValueError(
            "Router quant params have incompatible qweight shape: "
            f"input_size={input_size}, qweight_shape={tuple(qweight.shape)}"
        )
    pack_factor = input_size // qweight.shape[0]
    if pack_factor not in (4, 8):
        raise ValueError(
            "Router quant params imply unsupported pack factor: "
            f"input_size={input_size}, qweight_shape={tuple(qweight.shape)}"
        )
    num_bits = 32 // pack_factor

    if scales.shape[0] <= 0 or input_size % scales.shape[0] != 0:
        raise ValueError(
            "Router quant params have incompatible scales shape: "
            f"input_size={input_size}, scales_shape={tuple(scales.shape)}"
        )
