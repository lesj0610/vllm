...
class Gemma4Config(VerifyAndUpdateConfig):
    @staticmethod
    def verify_and_update_config(vllm_config: "VllmConfig") -> None:
        """Avoid mixed dense-attention backends on heterogeneous head sizes."""
        hf_text_config = vllm_config.model_config.hf_text_config
        cache_dtype = vllm_config.cache_config.cache_dtype
        head_dim = getattr(hf_text_config, "head_dim", None)
        global_head_dim = getattr(hf_text_config, "global_head_dim", None)

        # TurboQuant has its own backend-selection path keyed off KV cache
        # dtype. Preserve auto selection so CUDA can route to TURBOQUANT
        # directly instead of forcing TRITON_ATTN first.
