...
def _make_warmup_key(
    ...
    *,
    ...
    use_mm_prefix = mm_prefix_range_tensor is not None
    max_mm_ranges = (
        int(mm_prefix_range_tensor.shape[1])
...
def _make_mm_prefix_range_tensor(
    *,
    batch_size: int,
    device: torch.device,
) -> torch.Tensor:
    mm_prefix_range_tensor = torch.zeros(
        (batch_size, 1, 2), dtype=torch.int32, device=device
    )
    mm_prefix_range_tensor[:, 0, 1] = 1
...
def turboquant_decode_warmup(
    ...
    *,
    ...
    for layer, impl in _iter_turboquant_attention_layers(model):
        mm_prefix_range_tensors = [None]
        if getattr(layer, "use_mm_prefix", False):
            mm_prefix_range_tensors.append(
                _make_mm_prefix_range_tensor(
                    batch_size=max_num_decode_tokens,
                    device=device,
                )
            )

        for mm_prefix_range_tensor in mm_prefix_range_tensors:
