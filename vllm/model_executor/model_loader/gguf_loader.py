...
class GGUFModelLoader(BaseModelLoader):
    ...
    @staticmethod
    def _expand_gemma4_gguf_moe_tensor(
        name: str, weight: torch.Tensor
    ...
    def _get_weights_iterator(
        ...
        if hf_config.model_type == "gemma4":
            for name, weight in iterator:
                expanded = self._expand_gemma4_gguf_moe_tensor(name, weight)
                if expanded is not None:
                    for expanded_name, expanded_weight in expanded:
                        yield self._transform_gemma4_gguf_tensor_name_and_weight(
                            expanded_name, expanded_weight
                        )
                    continue
