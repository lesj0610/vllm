...
class INCGPTQRowParallelTailLinearMethod(LinearMethodBase):
    """Correctness-first fallback for GPTQ-family row-parallel linears whose
    local K shard crosses a quantization-group boundary.

    In this case, the shard needs one overlapping scale group from its neighbor.
    Loading scales/qzeros by floor-dividing local K drops that tail group and
    corrupts the output.
    """
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        shard_offset = getattr(
            layer, "input_size_per_partition", input_size_per_partition
        )
        shard_offset = qweight.tp_rank * shard_offset
        g_idx = torch.tensor(
            [
