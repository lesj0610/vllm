...
class INCConfig(QuantizationConfig):
    ...
    def apply_gptq_quant_layer(self, layer, prefix: str, backend: str = "auto"):
        ...
        if (
            ...
            # Gemma4 AutoRound row-parallel linears can produce shards that
            # straddle a GPTQ group boundary. Fall back to a correctness-first
            # path instead of using kernels that assume group-aligned inputs.
...
class INCGPTQRowParallelTailLinearMethod(LinearMethodBase):
    """Fallback for GPTQ-family linears with group-tail input shards."""
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        if self.sym:
            # The fallback handles the symmetric zero point via
            # self.weight_type.bias in _get_dequantized_weight(), so the large
            # packed qzeros tensor is replaced with a tiny placeholder after
            # loading.
            layer.qzeros = Parameter(
                torch.tensor([self.weight_type.bias], dtype=torch.int16).to(
