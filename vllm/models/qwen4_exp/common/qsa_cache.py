...
@functools.cache
def _has_cuda_metadata_builder() -> bool:
    return hasattr(torch.ops._C, "qsa_build_metadata")
...
def build_qsa_metadata(
    ...
    *,
    ...
    if _has_cuda_metadata_builder() and common_attn_metadata.query_start_loc.is_cuda:
        num_tokens = common_attn_metadata.num_actual_tokens
        num_mapped = int(common_attn_metadata.query_start_loc_cpu[-1])
        token_to_req = token_to_req_buffer[:num_tokens]
        logical_positions = logical_positions_buffer[:num_tokens]
        slot_mapping = slot_mapping_buffer[:num_tokens]
        # The buffer is sized by the owner; its whole length is the work bound.
        del request_capacity
