...
def test_doubled_bare_name_collapses():
    out = MuseGlimmerToolParser.extract_tool_calls(P, _call("get_weather.get_weather"), _req("get_weather"))
...
def test_namespaced_name_preserved():
    out = MuseGlimmerToolParser.extract_tool_calls(P, _call("weather.get"), _req("weather.get"))
...
def test_trailing_segment_unambiguous():
    ...
    out = MuseGlimmerToolParser.extract_tool_calls(P, _call("foo.get_weather"), _req("get_weather"))
...
def test_trailing_segment_ambiguous_left_alone():
    ...
    out = MuseGlimmerToolParser.extract_tool_calls(P, _call("x.get"), _req("weather.get", "time.get"))
    assert out.tool_calls[0].function.name == "x.get"


def test_no_registered_tools_passthrough():
    out = MuseGlimmerToolParser.extract_tool_calls(P, _call("get_weather.get_weather"), None)
    assert out.tool_calls[0].function.name == "get_weather.get_weather"


def test_exact_match_kept():
    out = MuseGlimmerToolParser.extract_tool_calls(P, _call("get_weather"), _req("get_weather"))
    assert out.tool_calls[0].function.name == "get_weather"


if __name__ == "__main__":
    for fn in [test_doubled_bare_name_collapses, test_namespaced_name_preserved,
               test_trailing_segment_unambiguous, test_trailing_segment_ambiguous_left_alone,
               test_no_registered_tools_passthrough, test_exact_match_kept]:
