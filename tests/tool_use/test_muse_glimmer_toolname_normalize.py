...
def test_trailing_segment_not_rewritten():
    # emitted foo.get_weather; registered get_weather (bare) -> left alone.
    # Trailing-segment matching is unsafe even when the leaf is unique: an
    # emitted `weather.get` against a registered {calendar.get} has a unique
    # leaf match and would silently dispatch the wrong tool. Only the doubled
    # head==tail form collapses -- see MuseGlimmerToolParser._normalize_name.
    out = MuseGlimmerToolParser.extract_tool_calls(
...
if __name__ == "__main__":
    for fn in [
        ...
        test_trailing_segment_not_rewritten,
