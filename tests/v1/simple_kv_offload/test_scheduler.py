...
def test_eager_store_and_load_roundtrip() -> None:
    ...
    # The last token is recomputed for logits, matching KVCacheManager prefix
    # cache semantics, so a two-block prompt yields one full CPU-hit block.
...
def test_lazy_store_and_load_roundtrip() -> None:
    ...
    # The final token is recomputed, so only one full block is externally loaded.
...
def test_partial_gpu_prefix_plus_cpu_load() -> None:
    ...
    # CPU has all 6 stored and returns hits starting from position 2. The final
    # token is still recomputed, so only blocks 2, 3, and 4 are externally hit.
