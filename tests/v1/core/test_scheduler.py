...
def test_schedule_ignores_request_that_can_never_fit_full_isl():
    scheduler = create_scheduler(
    ...
    assert output.ignored_reqs_by_client == {0: [request.request_id]}
    assert request.request_id not in scheduler.requests
    ...
    engine_outputs = scheduler.update_from_output(output, empty_output)

    assert 0 in engine_outputs
    assert len(engine_outputs[0].outputs) == 1
    assert engine_outputs[0].outputs[0].request_id == request.request_id
    assert engine_outputs[0].outputs[0].new_token_ids == []
    assert engine_outputs[0].outputs[0].finish_reason == FinishReason.LENGTH


def test_schedule_keeps_request_waiting_if_it_can_fit_later():
    scheduler = create_scheduler(
        max_num_batched_tokens=512,
        max_model_len=512,
        max_num_seqs=2,
    ...
    assert first_output.num_scheduled_tokens[first_request.request_id] == 192
    ...
    second_request = create_requests(
        num_requests=1,
        num_tokens=160,
        req_ids=["second"],
        max_tokens=1,
    ...
    assert second_output.total_num_scheduled_tokens == 1
    assert second_output.num_scheduled_tokens == {first_request.request_id: 1}
    assert second_output.finished_req_ids == set()
    assert second_output.ignored_reqs_by_client is None
    assert second_request.request_id in scheduler.requests
    assert scheduler.requests[second_request.request_id].status == RequestStatus.WAITING
