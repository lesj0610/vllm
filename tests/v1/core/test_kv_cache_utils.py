...
def test_request_constant_mamba_full_and_piecewise_downgrades_to_piecewise():
    vllm_config = make_request_constant_vllm_config(max_num_seqs=4)
    attention_spec = new_kv_cache_spec(
        block_size=4,
        num_kv_heads=1,
...
def test_num_blocks_override_reserves_request_constant_for_minimal_profile():
    vllm_config = make_request_constant_vllm_config(max_num_seqs=4)
    attention_spec = new_kv_cache_spec(
        block_size=4,
        num_kv_heads=1,
    ...
    min_profile_blocks = 3

    vllm_config.cache_config.num_gpu_blocks_override = min_profile_blocks
    available_memory, profiled_blocks = (
        kv_cache_utils._get_effective_available_memory_for_num_blocks_override(
            vllm_config,
            groups,
            available_memory=0,
            num_blocks_override=min_profile_blocks,
        )
    )

    assert profiled_blocks == 0
    assert available_memory == (
        reserved_bytes + min_profile_blocks * attention_spec.page_size_bytes
    )
    ...
    assert config.num_blocks == min_profile_blocks
    assert config.pool_configs[0].memory_model == MemoryModel.TOKEN_PROPORTIONAL
    assert config.pool_configs[0].num_blocks == min_profile_blocks
    assert config.pool_configs[1].memory_model == MemoryModel.REQUEST_CONSTANT
    assert config.pool_configs[1].num_blocks == request_constant_num_blocks
    assert config.kv_cache_tensors == [
        KVCacheTensor(
            size=attention_spec.page_size_bytes * min_profile_blocks,
            shared_by=["attn"],
        ),
        KVCacheTensor(size=reserved_bytes, shared_by=["state"]),
    ]
...
def test_request_constant_only_config_accepts_exact_reservation():
    vllm_config = make_request_constant_vllm_config(max_num_seqs=4)
    request_constant_spec = new_request_constant_spec(
        num_speculative_blocks=1,
        page_size_padded=16,
    ...
    groups = [KVCacheGroupSpec(["state"], request_constant_spec)]

    vllm_config.cache_config.num_gpu_blocks_override = 3
    available_memory, profiled_blocks = (
        kv_cache_utils._get_effective_available_memory_for_num_blocks_override(
            vllm_config,
            groups,
            available_memory=0,
            num_blocks_override=3,
        )
    )

    assert profiled_blocks == 0
    assert available_memory == reserved_bytes
