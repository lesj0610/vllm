...
def test_get_max_concurrency_for_kv_cache_config():
    ...
    # whole pool blocks, so a request needs 1024 + 129 = 1153 blocks,
    # regardless of the second group holding only 2 layers. One pool block
    # remains reserved for the null block.
...
def _new_request_constant_mamba_test_inputs(
    ...
    # The engine core normally resolves this once before KV cache planning.
    vllm_config.cache_config.kv_cache_layout = "LBNHC"
...
def test_request_constant_mamba_uses_shared_pool():
    ...
    # Both groups overlay one backing allocation: a tensor per layer, all
    # sized to the same buffer that allocate_kv_cache asserts on.
    expected_size = kv_cache_specs[0]["attn"].page_size_bytes * (
    ...
    assert {tensor.offset for tensor in kv_cache_config.kv_cache_tensors} == {0}
...
def test_auto_fit_max_model_len_with_hybrid():
    ...
    # The mamba pool is request-constant, so it needs blocks_per_request for
    # every concurrent sequence. One extra block on top of that: the pool
