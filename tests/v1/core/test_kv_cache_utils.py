...
def _get_group_pool_ids(config: KVCacheConfig) -> list[int]:
    return list(config.group_to_pool_id)


def _get_tensor_pool_id(config: KVCacheConfig, tensor: KVCacheTensor) -> int:
    return kv_cache_utils._get_tensor_pool_id(config, tensor)


def _get_tensor_pool_ids(config: KVCacheConfig) -> list[int]:
    return [_get_tensor_pool_id(config, tensor) for tensor in config.kv_cache_tensors]
...
def test_multi_pool_kv_cache_manager_uses_pool_local_blocks():
    ...
    assert kv_cache_manager.usage == pytest.approx(1 / 2)
...
def test_get_kv_cache_config_one_worker():
    ...
    assert _get_group_pool_ids(kv_cache_config_hybrid) == [
        0,
        1,
    ]
    assert _get_tensor_pool_ids(kv_cache_config_hybrid) == [
        0,
        1,
    ]
...
def test_tq_native_mixed_kv_cache_uses_hybrid_page_groups():
    ...
    assert kv_cache_config.num_blocks == kv_cache_config.kv_cache_pools[0].num_blocks
    assert _get_group_pool_ids(kv_cache_config) == [
        expected_page_sizes.index(group.kv_cache_spec.page_size_bytes)
        for group in kv_cache_config.kv_cache_groups
    ]
    for tensor in kv_cache_config.kv_cache_tensors:
        pool = kv_cache_config.kv_cache_pools[
            _get_tensor_pool_id(kv_cache_config, tensor)
        ]
...
def test_multi_pool_kv_cache_configs_shrink_each_pool_across_workers():
    ...
    for kv_cache_config in kv_cache_configs:
        assert kv_cache_config.is_multi_pool
        assert (
            kv_cache_config.num_blocks == kv_cache_config.kv_cache_pools[0].num_blocks
        )
        for tensor in kv_cache_config.kv_cache_tensors:
            pool = kv_cache_config.kv_cache_pools[
                _get_tensor_pool_id(kv_cache_config, tensor)
