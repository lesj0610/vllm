...
def test_turboquant_physical_page_size_uses_slot_size() -> None:
    full_spec = _tq_full_attention_spec()
    sliding_spec = _tq_sliding_window_spec()

    assert (
        full_spec.physical_page_size_bytes
        == full_spec.block_size * full_spec.num_kv_heads * full_spec.tq_slot_size
    )
    assert (
        sliding_spec.physical_page_size_bytes
        == sliding_spec.block_size
        * sliding_spec.num_kv_heads
        * sliding_spec.tq_slot_size
    )
...
def test_uniform_type_physical_page_size_sums_children() -> None:
    ...
    tq_spec = _tq_full_attention_spec()
    ...
    assert uniform_spec.physical_page_size_bytes == (
        full_spec.physical_page_size_bytes + tq_spec.physical_page_size_bytes
    )
