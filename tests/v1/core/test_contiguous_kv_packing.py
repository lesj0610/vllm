...
# ---------------------------------------------------------------------------
# Qwen4Exp CSA-linear interleaved packing (ported to the unified overlay
...
class TestInterleavedPacking:
    ...
    def test_shared_pool_sizing_and_layer_coverage(self):
        config = _shared_layout_config()
        specs = _make_csa_linear_specs()
        groups = get_kv_cache_groups(config, specs)

        assert _pool_bytes_per_block(groups) == BYTES_PER_BLOCK
        ...
        assert sizes == {BYTES_PER_BLOCK * 32}
        covered = {
            name
