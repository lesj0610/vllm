...
def test_turboquant_decode_warmup_keeps_distinct_sliding_windows() -> None:
    first = _FakeAttention(impl=_FakeTurboQuantAttentionImpl(sliding_window=None))
    second = _FakeAttention(impl=_FakeTurboQuantAttentionImpl(sliding_window=1024))
    model = torch.nn.Sequential(first, second)
...
def test_turboquant_decode_warmup_adds_mm_prefix_variant() -> None:
    impl = _FakeTurboQuantAttentionImpl()
    model = torch.nn.Sequential(_FakeAttention(impl=impl, use_mm_prefix=True))
    ...
    base_metadata = impl.decode_calls[0]["attn_metadata"]
    mm_prefix_metadata = impl.decode_calls[1]["attn_metadata"]
    assert base_metadata.mm_prefix_range_tensor is None
    assert mm_prefix_metadata.mm_prefix_range is None
    assert mm_prefix_metadata.mm_prefix_range_tensor.shape == (4, 1, 2)
    assert mm_prefix_metadata.mm_prefix_range_tensor.dtype == torch.int32
    assert mm_prefix_metadata.mm_prefix_range_tensor.tolist() == [
        [[0, 1]],
        [[0, 1]],
        [[0, 1]],
        [[0, 1]],
