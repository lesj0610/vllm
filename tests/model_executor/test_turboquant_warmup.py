...
from vllm.v1.attention.ops import triton_turboquant_decode  # noqa: E402
...
class _FakeTurboQuantAttentionImpl:
    def __init__(
        ...
        *,
        ...
        self.dequant_calls: list[dict[str, Any]] = []
    ...
    def _dequant_cached_kv(
        ...
        output = torch.empty(
            (cache_len, self.num_kv_heads, self.head_size),
            dtype=output_dtype,
...
def patch_turboquant_types(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        turboquant_warmup.torch.accelerator, "synchronize", lambda: None
    )
    monkeypatch.setattr(
        turboquant_warmup, "is_workspace_manager_initialized", lambda: False
    )
    monkeypatch.setattr(
        turboquant_warmup,
...
def test_turboquant_kernels_do_not_specialize_runtime_strides() -> None:
    assert set(triton_turboquant_decode._tq_decode_stage1.do_not_specialize) >= {
        "stride_qb",
        "stride_qh",
        "stride_bt_b",
        "stride_mid_b",
    ...
    assert set(triton_turboquant_decode._tq_decode_stage2.do_not_specialize) >= {
        "stride_mid_ob",
        "stride_mid_os",
        "stride_obs",
        "stride_lse_bs",
    }
    assert set(triton_turboquant_decode._tq_full_dequant_kv.do_not_specialize) >= {
        "stride_ko_h",
        "stride_vo_h",
        "stride_cache_block",
        "stride_bt_b",
...
def test_turboquant_warmup_skips_non_tq_layers() -> None:
    layer = _FakeAttention(kv_cache_dtype="fp8")
    model = torch.nn.Sequential(layer)

    turboquant_warmup.turboquant_attention_warmup(
        model,
    ...
    assert layer.impl.dequant_calls == []
    assert layer.impl.ensure_calls == 0


def test_turboquant_warmup_builds_runtime_shaped_decode_inputs(
    ...
    monkeypatch.setattr(
        turboquant_warmup, "triton_turboquant_decode_attention", fake_decode_launcher
    )

    impl = _FakeTurboQuantAttentionImpl(max_num_kv_splits=64)
    model = torch.nn.Sequential(_FakeAttention(impl=impl))
    runtime_kv_cache = torch.zeros(
        (3, 32, impl.num_kv_heads, impl.tq_config.slot_size_aligned),
        dtype=torch.uint8,
    )
...
def test_turboquant_warmup_builds_all_runtime_shapes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        turboquant_warmup, "is_workspace_manager_initialized", lambda: False
...
def test_turboquant_warmup_also_warms_full_dequant(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        turboquant_warmup, "is_workspace_manager_initialized", lambda: True
    )
    impl = _FakeTurboQuantAttentionImpl()
    model = torch.nn.Sequential(_FakeAttention(impl=impl))
    ...
    assert len(impl.dequant_calls) == 1
    dequant_call = impl.dequant_calls[0]
    assert dequant_call["block_table"].shape == (1, 17)
    assert dequant_call["block_table"].tolist()[0][:2] == [1, 0]
    assert dequant_call["cache_len"] == 32
    assert dequant_call["output_dtype"] == torch.float16


def test_turboquant_warmup_deduplicates_compile_key(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        turboquant_warmup, "is_workspace_manager_initialized", lambda: False
...
def test_turboquant_warmup_keeps_distinct_compile_keys(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        turboquant_warmup, "is_workspace_manager_initialized", lambda: False
...
def test_kernel_warmup_passes_turboquant_runtime_constants(
    ...
    monkeypatch.setattr(kernel_warmup, "turboquant_attention_warmup", fake_tq_warmup)
    monkeypatch.setattr(kernel_warmup, "fused_moe_wna16_warmup", lambda *args: None)
...
def test_kernel_warmup_uses_v2_block_table_constants(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls = []
